...
def sift_matching(des1, des2):
    m, _ = des1.shape   # Get the number of descriptors in des1
    n, _ = des2.shape   # Get the number of descriptors in des2
    index = np.zeros(m, dtype=np.int32)  # Initializing the output index array with length m

    for i in range(m): # Get the index of the best matching descriptor in des2 for each descriptor in des1
        # Calculate L2 distances between des1[i, :] and all descriptors in des2
        distances = np.linalg.norm(des2 - des1[i, :], axis=1)
        
        # Find the index of the descriptor in des2 with the minimum L2 distance
